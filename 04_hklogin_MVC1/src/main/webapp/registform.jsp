		...
		for (let i = 2; i < inputs.length; i++) {
			inputs[i].onfocus = function() {
				...
				if (isIdChk==null||isIdChk=='n') {
					alert("아이디 중복 체크를 먼저 확인하세요.")
					inputs[1].focus();
				}
			}
		}
	}
	// form 전송시 패스워드가 맞는지 확인 후 가입 진행
	function isPW(formEle) {
		
		// password1, password2의 값이 동일한지 확인
		if (formEle.password1.value != formEle.password2.value) {
			alert("비밀번호를 확인하세요")
			formEle.password1.value="";
			formEle.password2.value="";
			formEle.password1.focus();
			return false;
		}
...
<div id="registForm">
	...
	<form action="userController.jsp" method="post" onsubmit="return isPW(this)">
		...
		<input type="password" name="password1" required="required" placeholder="PASWORD" />
		<input type="password" name="password2" required="required" placeholder="PASSWORD 확인" />
